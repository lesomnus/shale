...
// its own bytes; it gives the attempt up and the lamina gets another,
...
func TestForeignBytes(t *testing.T) {
	...
	up := &producer.Uploader{Cfg: producer.UploadConfig{ResumeTimeout: 5 * time.Second, PlacementRetries: 2}, Laminae: laminae, Log: slog.Default(), Mode: api.UploadMode_UPLOAD_MODE_BUFFERED}
	...
	require.NoError(t, err)
	require.Len(t, tl.GetSources()[0].GetLaminae(), 1)
	require.Equal(t, body, fetch(t, tl.GetSources()[0].GetLaminae()[0].GetUrl()), "the new segment, whole")
}
