...
func TestOneWriterPerKey(t *testing.T) {
	...
	case r := <-first:
		if r != nil {
			require.NotEqual(t, http.StatusCreated, r.StatusCode, "the older request did not complete the lamina")
		}
	...
	require.Equal(t, body, fetch(t, tl.GetSources()[0].GetLaminae()[0].GetUrl()))
}
