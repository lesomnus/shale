...
func (_q *AttemptQuery) QueryTenant() *TenantQuery {
...
func (_q *AttemptQuery) QuerySite() *SiteQuery {
...
func (_q *AttemptQuery) QuerySink() *SinkQuery {
...
func (_q *AttemptQuery) QueryNode() *NodeQuery {
...
// All executes the query and returns a list of Attempt entities.
...
func (_q *AttemptQuery) WithTenant(opts ...func(*TenantQuery)) *AttemptQuery {
	query := (&TenantClient{config: _q.config}).Query()
...
func (_q *AttemptQuery) WithSite(opts ...func(*SiteQuery)) *AttemptQuery {
	query := (&SiteClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AttemptQuery) WithSink(opts ...func(*SinkQuery)) *AttemptQuery {
	query := (&SinkClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AttemptQuery) WithNode(opts ...func(*NodeQuery)) *AttemptQuery {
	query := (&NodeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AttemptQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attempt, error) {
	...
	if query := _q.withTenant; query != nil {
		if err := _q.loadTenant(ctx, query, nodes, nil,
			func(n *Attempt, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSite; query != nil {
		if err := _q.loadSite(ctx, query, nodes, nil,
			func(n *Attempt, e *Site) { n.Edges.Site = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withSink; query != nil {
		if err := _q.loadSink(ctx, query, nodes, nil,
			func(n *Attempt, e *Sink) { n.Edges.Sink = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withNode; query != nil {
		if err := _q.loadNode(ctx, query, nodes, nil,
			func(n *Attempt, e *Node) { n.Edges.Node = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AttemptQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Attempt, init func(*Attempt), assign func(*Attempt, *Tenant)) error {
...
func (_q *AttemptQuery) loadSite(ctx context.Context, query *SiteQuery, nodes []*Attempt, init func(*Attempt), assign func(*Attempt, *Site)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Attempt)
	for i := range nodes {
		fk := nodes[i].SiteId
...
func (_q *AttemptQuery) loadSink(ctx context.Context, query *SinkQuery, nodes []*Attempt, init func(*Attempt), assign func(*Attempt, *Sink)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Attempt)
	for i := range nodes {
		fk := nodes[i].SinkId
...
func (_q *AttemptQuery) loadNode(ctx context.Context, query *NodeQuery, nodes []*Attempt, init func(*Attempt), assign func(*Attempt, *Node)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Attempt)
	for i := range nodes {
		fk := nodes[i].NodeId
...
func (_q *AttemptQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withTenant != nil {
			_spec.Node.AddColumnOnce(attempt.FieldTenantId)
		}
		if _q.withSite != nil {
		...
		if _q.withNode != nil {
			_spec.Node.AddColumnOnce(attempt.FieldNodeId)
		}
	}
