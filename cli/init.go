...
func NewCmdInit(c *cmd.Config) *xli.Command {
	return &xli.Command{
		...
		Handler: xli.OnRun(func(ctx context.Context, self *xli.Command, next xli.Next) error {
			...
			tenant := flagOr(self, "tenant", "acme")
			admin := flagOr(self, "admin", "admin")
			operator := flagOr(self, "operator", "ops")
			...
			keys := core.NewKeys(s.Kek, s.Ungated, nil)
			if s.Kek == nil {
