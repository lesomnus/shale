...
| **Storage Node** | Host that stores and serves laminae |
| **Segment** | What a producer makes: one source's bytes between two cuts and the time span they cover, nothing else ([§38.2](15-producer.md#382-cutting-segments)) |
| **Lamina** (pl. laminae) | What the cluster keeps about a segment: the immutable stored unit with its state, dates and place on a sink, from allocation to `STORED`, `LOST` or `DELETED` ([§8](02-data-model.md#8-state-model)). Named after the thin layers shale splits into |
...
| **Reader** | Host that reads laminae, typically a media server; `shale serve reader` is the agent that holds its certificate |
...
| **Source** | A producer of laminae (a camera, or one stream of data) |
...
| **Lamina** | Immutable unit of storage (a video segment) |
| **Lamina ID** | Global identity of a logical lamina |
| **Attempt ID** | Identity of one write attempt of a lamina |
| **Lamina Key** | Path of a lamina within its sink |
| **Location** | `(sink_id, lamina_key)` |
| **Allocation** | Result of placement: a lamina and its ranked candidates, each with an attempt and a token |
| **Placement** | Choosing a sink for a new lamina |
...
| **Sink** | A directory where a node stores laminae; unit of placement and location |
...
| **Incomplete Lamina** | A committed lamina finalized from an abandoned live upload; its tail is missing and its end is estimated |
...
| **Commit** | Lamina durable in one sink; signalled to the producer by the final `201` |
| **Event** | What a node reports to the CP: `LaminaStored`, `LaminaDeleted`, `LaminaMissing` |
...
| **Duplicate** | A second stored attempt of an already committed lamina |
| **Orphan** | A file in a sink unknown to the index |
| **Lost Lamina** | A lamina that could not be stored, or whose file is gone |
| **date_expired** | Time from which the lamina may be deleted when space is needed |
| **date_deleted** | Time from which the lamina is deleted; read like a certificate expiry |
| **Sweep** | The node's daily proposal of every file whose xattr `date_deleted` has passed |
| **Reschedule** | Changing a lamina's dates, to preserve it longer or delete it sooner; audited; pushed to the node at once |
| **Row Retention** | How long rows of deleted, lost, and never-used laminae are kept for readers |
| **Data Time** | `date_started`/`date_ended`: the span a lamina's data covers, declared by the producer |
...
| **Fair Share** | GC ordering that reclaims expired laminae of over-share tenants first |
...
| **Lazy GC** | Deleting expired laminae only under space pressure |
...
| **Access Token** | CP-signed (Ed25519) grant for one operation at one host: put or get on one lamina at a node, publish or view on a relay; carries the actor; a presigned URL carries it |
...
| **Tenant** | Owner of sets, sources, laminae, people, producers, and readers; one per organization, and a single-organization cluster has exactly one |
