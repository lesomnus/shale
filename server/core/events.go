...
// Events a node pushes (§34.9), applied idempotently: LaminaStored is the
// node's final word about what is on its device (§14), LaminaDeleted
// confirms a deletion (§21.2), LaminaMissing says a key is gone.
...
func (s Core) applyStored(ctx context.Context, own api.Server, nodeId pdid.Id, ev *api.LaminaStored) error {
	atId, err := pdid.From(ev.GetAttemptId())
	if err != nil {
		return invalid("attempt_id", err.Error())
	}
	objId, err := pdid.From(ev.GetLaminaId())
	if err != nil {
		return invalid("lamina_id", err.Error())
	}
	now := s.d.now()
	...
	if obj == nil {
		...
		if rec == nil || len(rec.GetTenantId()) == 0 || len(rec.GetSetId()) == 0 || len(rec.GetSourceId()) == 0 {
			return failed("lamina %s is unknown and the event carries no record", objId)
		}
		...
		if _, err := own.Lamina().Add(ctx, add.Build()); err != nil {
			return err
		}
		...
		obj = &ent.Lamina{
			...
			State: int32(api.LaminaState_LAMINA_STATE_PENDING), DateStarted: add.DateStarted.AsTime(), DateExpired: add.DateExpired.AsTime(), DatesSynced: true,
		}
		if len(rec.GetSiteId()) > 0 {
	...
	if at == nil {
		...
		at = &ent.Attempt{
			Id: atId.Uuid(), TenantId: obj.TenantId, SiteId: obj.SiteId, LaminaId: objId.Uuid(), SinkId: mustId(ev.GetSinkId()).Uuid(), NodeId: nodeId.Uuid(),
	...
	if obj.State == int32(api.LaminaState_LAMINA_STATE_DELETING) || obj.State == int32(api.LaminaState_LAMINA_STATE_DELETED) {
		// The lamina was deleted meanwhile; this file is a duplicate GC
	...
	patch := api.LaminaPatchRequest_builder{
		...
		Sink:             api.SinkRef_builder{Id: ev.GetSinkId()}.Build(),
		LaminaKey:        z.Ptr(ev.GetLaminaKey()),
		State:            z.Ptr(api.LaminaState_LAMINA_STATE_COMMITTED),
	...
	if _, err := own.Lamina().Patch(ctx, patch.Build()); err != nil {
		return err
	}
...
func (s Core) observe(ctx context.Context, own api.Server, obj *ent.Lamina, ev *api.LaminaStored) error {
...
// applyDeleted confirms a deletion: the lamina at that location is DELETED,
// or a duplicate's file is forgotten (§21.2).
func (s Core) applyDeleted(ctx context.Context, own api.Server, nodeId pdid.Id, ev *api.LaminaDeleted) error {
	...
	if obj.State == int32(api.LaminaState_LAMINA_STATE_DELETED) {
		return nil
	}
	...
	if _, err := own.Lamina().Patch(ctx, api.LaminaPatchRequest_builder{
		Ref:              api.LaminaRef_builder{Id: obj.Id[:]}.Build(),
		State:            z.Ptr(api.LaminaState_LAMINA_STATE_DELETED),
...
// applyMissing marks a lamina lost, or confirms a deletion in progress
// (§14).
func (s Core) applyMissing(ctx context.Context, own api.Server, nodeId pdid.Id, ev *api.LaminaMissing) error {
	if err := s.sinkOnNode(ctx, nodeId, ev.GetSinkId(), nil); err != nil {
		return err
	}
	sid := mustId(ev.GetSinkId())
	...
	case api.LaminaState_LAMINA_STATE_DELETING:
		if _, err := own.Lamina().Patch(ctx, api.LaminaPatchRequest_builder{
			Ref: api.LaminaRef_builder{Id: obj.Id[:]}.Build(), State: z.Ptr(api.LaminaState_LAMINA_STATE_DELETED),
	...
	case api.LaminaState_LAMINA_STATE_COMMITTED:
		if _, err := own.Lamina().Patch(ctx, api.LaminaPatchRequest_builder{
			Ref: api.LaminaRef_builder{Id: obj.Id[:]}.Build(), State: z.Ptr(api.LaminaState_LAMINA_STATE_LOST),
			DateFinished: timestamppb.New(now), DateUpdatedForce: z.Ptr(true),
		}.Build()); err != nil {
			return err
		}
